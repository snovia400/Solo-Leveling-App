...
def sign_up():
    ...
    with open("user_data.txt", "a") as file:  
        file.write(f"{username},{passw},{email}".strip() + "\n")
...
def login():
    ...
    for user in users:
        org_user, org_password, org_email = user.strip().split(",")
        if username_input == org_user and password_input == org_password:
            print(f"Welcome, {org_user}!")
            return  # exit after successful login

    print("Invalid username or password.")

# Main Menu
choice = input("Would you like to register or login? (register/login): ").lower()
